  ...
  <div class="border border-[#39bcd4] py-6 px-10 shadow-2xl">
    ...
    <div class="mt-8">
      <div v-for="(row, rowIndex) in board" class="flex border-b-2 last:border-b-0 border-stone-300">
        <div v-for="(square, columnIndex) in row" class="border-r-2 last:border-r-0 border-stone-300 p-0.5">
          <button type="button" class="w-16 h-16 flex items-center justify-center" @click="onMove(rowIndex, columnIndex)">
    ...
    <div class="mt-8 flex items-center justify-center space-x-4">
      <div class="flex rounded-full border border-[#e0e5e6] p-0.5">
        <button
          type="button"
          class="rounded-full bg-[#a4b5b8] text-[#e0e5e6] hover:text-slate-100 transition-colors duration-200 p-2"
          title="Reset game"
          @click="onReset"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="w-5 h-5">
            <path
              ...
              stroke-width="2"
              d="m20.956 9.748 1.298-3.618M20.956 9.748l-3.793-.625M20.162 8.282A8.749 8.749 0 0 0 3.48 10.85M3.546 14.252 2.247 17.87M3.546 14.252l3.792.625M4.34 15.718a8.749 8.749 0 0 0 16.683-2.567"
            />
          </svg>
        </button>
      </div>

      <span class="border border-stone-200 rounded-full px-3 py-1.5 text-[#999999] text-xs font-bold uppercase"> 2 players </span>

      <div class="flex rounded-full border border-[#e0e5e6] p-0.5">
        <a
          href="https://github.com/MirasMustimov/tic-tac-toe-demo"
          target="blank"
...
let winnerIsDefined = computed(() => {
  ...
  let winsByDiagonal =
    board.value.every((row, index) => row[0 + index] === move.sign) ||
...
let onMove = (row: number, column: number) => {
  ...
  if (gameIsOver.value && lastMove.value) {
    winnerIsDefined.value ? gameResultStats.value[lastMove.value.sign]++ : gameResultStats.value.draw++
  }
}
